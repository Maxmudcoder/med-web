    ...
    open && typeof document !== 'undefined'
      ? createPortal(
          <div
            ref={menuRef}
            role="dialog"
            aria-label="Yangiliklar va e'lonlar"
            className="fixed z-[400] w-[min(calc(100vw-2rem),22rem)] rounded-2xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-card)] py-3 shadow-2xl shadow-black/40 backdrop-blur-xl md:w-[22rem]"
  ...
  return (
    <div className="relative z-[110] flex items-center">
      <button
        ...
        type="button"
        onClick={togglePanel}
        className={`relative flex h-11 w-11 shrink-0 touch-manipulation items-center justify-center rounded-xl border bg-[var(--color-bg-card)] text-[var(--color-text)] transition hover:border-teal-500/50 hover:text-teal-400 sm:w-12 ${
          unseenCount > 0
